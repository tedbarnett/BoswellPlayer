...
var stateHandlers = {
    startModeIntentHandlers : Alexa.CreateStateHandler(constants.states.START_MODE, {
        ...
        'AMAZON.HelpIntent' : function () {
            var message = 'Happy to Help. You can say, playback, to begin listening to recorded interviews. While recordings are playing you can say ALEXA NEXT to skip to the next or ALEXA PREVIOUS or ALEXA CANCEL or ALEXA LOOP or ALEXA SHUFFLE.';
        ...
        // THB: Add in a "ChangeUserIntent" to listen to stories from friends
        'Unhandled' : function () {
    ...
    playModeIntentHandlers : Alexa.CreateStateHandler(constants.states.PLAY_MODE, {
        ...
        'LaunchRequest' : function () {
          let _this = this;
            ...
            if (_this.attributes['playbackFinished']) {
                _this.handler.state = constants.states.START_MODE;
                message = 'Welcome back to Boswell Memories. You can say, playback, to begin listening to recorded interviews.';
                ...
                reprompt = constants.resumeReprompt;
            }
        ...
        'PlayAudio': function () {
          let _this = this;
            audioData(function(err, list) {
              ...
              var reprompt = constants.resumeReprompt;
              controller.play.call(_this)
          });
        },
        ...
        'SessionEndedRequest' : function () {
            // No session ended logic
            var message = 'Thanks for listening.  So long for now.';
            this.response.speak(message).listen(message);
            this.emit(); // THB: Get rid of responseReady so it ends?
        },
        'Unhandled' : function () {
    ...
    resumeDecisionModeIntentHandlers : Alexa.CreateStateHandler(constants.states.RESUME_DECISION_MODE, {
        ...
        'LaunchRequest' : function () {
          let _this = this;
            audioData(function(err, list) {
            var message = 'Welcome back.  You were listening to ' + list[_this.attributes['playOrder'][_this.attributes['index']]].title +
                ' Would you like to resume?';
            var reprompt = constants.resumeReprompt;
            _this.response.speak(message).listen(reprompt);
            _this.emit(':responseReady');
    ...
    return {
        play: function () {
          ...
          audioData(function(err, list) {
            ...
            // if (_this.attributes['playbackFinished']) { // Disabled if test for Boswell
            ...
            var playBehavior = 'REPLACE_ALL';
              ...
              if (canThrowCard.call(_this)) {
                  ...
                  var cardContent = 'TRANSCRIPTION: ' + podcast.transcription;
          ...
          let _this = this;
            ...
            if (index === -1) {
                if (_this.attributes['loop']) {
                    ...
                    var message = 'You have reached the most recent interview.';
                    _this.response.speak(message).audioPlayerStop();
                    return _this.emit(':responseReady');
                }
            }
